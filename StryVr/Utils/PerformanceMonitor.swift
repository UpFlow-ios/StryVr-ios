...
final class PerformanceMonitor: ObservableObject {
    ...
    func checkSystemResources() -> Bool {
        ...
        if memoryUsagePercentage > 80 {
            logger.error("🚨 Critical memory usage: \(String(format: "%.1f%%", memoryUsagePercentage))")
            return false
        }
...
extension View {
    ...
    func monitorPerformance(_ name: String) -> some View {
        ...
        return self.onAppear {
            ...
            PerformanceMonitor.shared.logPerformanceEvent("View appeared: \(name)", duration: duration)
        }
    }
} 