...
struct OktaConfig {
    ...
    static var authorizationEndpoint: URL {
        guard let url = URL(string: "https://\(domain)/oauth2/v1/authorize") else {
    ...
    static var tokenEndpoint: URL {
        guard let url = URL(string: "https://\(domain)/oauth2/v1/token") else {
            fatalError("Invalid token endpoint URL")
        }
        return url
    }
    ...
    static var userInfoEndpoint: URL {
        guard let url = URL(string: "https://\(domain)/oauth2/v1/userinfo") else {
            fatalError("Invalid user info endpoint URL")
        }
        return url
    }
    ...
    static var workforceIdentityEndpoint: URL {
        guard let url = URL(string: "https://\(domain)/api/v1/users/me") else {
            fatalError("Invalid workforce identity endpoint URL")
        }
        return url
    }
