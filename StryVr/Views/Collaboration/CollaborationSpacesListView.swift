    ...
    private var createSpacePromptSection: some View {
        VStack(spacing: Theme.Spacing.medium) {
            ...
            VStack(spacing: 8) {
                ...
                Text("Build a collaborative environment where your team can share skills and bridge gaps together")
                    .font(Theme.Typography.caption)
                    .foregroundColor(Theme.Colors.textSecondary)
        ...
        Button(action: action) {
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 16)
                    .stroke(
                        LinearGradient(
                            colors: [Theme.Colors.neonYellow.opacity(0.5), space.color.opacity(0.5)],
                            startPoint: .leading,
                            endPoint: .trailing
                        ),
                        lineWidth: 1
    ...
    func loadSpaces() {
        ...
        allSpaces = [
            CollaborationSpace(
                name: "Innovation Hub",
                activeMembers: 12,
                skillsBeingShared: ["AI/ML", "Product Design", "Strategy", "Data Science", "UX Research"],
    ...
    func filterSpaces(by searchText: String, category: SpaceCategory) {
        ...
        if category != .all {
            filtered = filtered.filter { space in
                ...
                switch category {
                case .innovation:
                    return space.name.contains("Innovation") || space.skillsBeingShared.contains("AI/ML")
                case .engineering:
                    return space.skillsBeingShared.contains { $0.contains("Development") || $0.contains("Engineering") }
                case .design:
                    return space.skillsBeingShared.contains { $0.contains("Design") || $0.contains("UX") }
        ...
        if !searchText.isEmpty {
            filtered = filtered.filter { space in
                ...
                space.skillsBeingShared.contains { $0.localizedCaseInsensitiveContains(searchText) }
            }
        }
    ...
    private func loadAIRecommendations() {
        aiRecommendations = [
            AISpaceRecommendation(
                ...
                description: "Your AI/ML skills would be valuable here. 3 members are looking for your expertise.",
                confidence: 0.92,
            ...
            AISpaceRecommendation(
                ...
                description: "Found 2 iOS developers in Mobile Excellence who match your skill level.",
                confidence: 0.87,
                targetSpaceId: nil
            ),
            AISpaceRecommendation(
                ...
                description: "No active marketing collaboration spaces found. Create one to connect with marketing professionals.",
                confidence: 0.75,
                targetSpaceId: nil
            )
        ]
