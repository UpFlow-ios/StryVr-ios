...
class OktaIntegrationService: ObservableObject {
    ...
    private let oktaDomain = "dev-72949354.okta.com"
    private let clientId = "YOUR_OKTA_CLIENT_ID" // Store securely
    private let redirectUri = "stryvr://okta-callback"
    private let scope = "openid profile email workforce_identity"
    ...
    private func fetchHRData(accessToken: String) async throws -> OktaHRData {
        guard let url = URL(string: "https://\(oktaDomain)/api/v1/users/me") else {
            throw OktaError.invalidURL
        }
    ...
    private func exchangeCodeForTokens(_ code: String) async throws -> OktaTokens {
        guard let url = URL(string: "https://\(oktaDomain)/oauth2/v1/token") else {
            throw OktaError.invalidURL
        }
    ...
    private func fetchUserInfo(accessToken: String) async throws -> OktaUserInfo {
        guard let url = URL(string: "https://\(oktaDomain)/oauth2/v1/userinfo") else {
            throw OktaError.invalidURL
        }
    ...
    private func buildAuthURL() -> URL {
        ...
        components.host = oktaDomain
        components.path = "/oauth2/v1/authorize"
